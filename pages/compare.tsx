...
const CompareProduct = ({ product }: { product: dataCartProduct }) => {
    ...
    const isFavorite = useIsFavorites(favoritesProducts, product);
    ...
    return (
        <div className={style.containerCompare}>
            <div className={style.logoImage} onClick={() => router.push(`/category/category1/${product.id}`)}>
                <Image src={product.images[0]} alt='imageLogo' />
            </div>
...
const Compare = () => {
    ...
    return (
        <div className={style.container}>
            <HeadComponent description='Презентация сайта магазина одежды. Сравнение товаров' viewport='width=device-width, initial-scale=1' />
