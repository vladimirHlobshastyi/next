...
const NavbarInUserComponent = () => {
  ...
  return (
    <div className={style.navigate}>
      <div className={titleClass} onClick={changeDropdown}>
        <span>Линый кабинет</span>
        {mobileDropDownIcon}
      </div>
      <div className={isDropdownClass}>
        <div className={historyOrdersClass} onClick={pushToHistoryOfOrders}>
          История заказов
        </div>
        <div className={adressClass} onClick={pushToAdress}>
          Адрес доставки
        </div>
        <div className={bonusesClass} onClick={pushToBonuses}>
          Скидки и бонусы
        </div>
        <div className={contactsClass} onClick={pushToContacts}>
          Контактные данные
        </div>
        <div className={style.navigateEl} onClick={logoutFunc}>
          Выход
        </div>
      </div>
    </div>
